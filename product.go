...
func filterProducts(db *sql.DB, name string) ([]product, error) {
	rows, err := db.Query(
...
func getSumOfProducts(db *sql.DB, start, count int) (float64, error) {
	sum := 0.0

	err := db.QueryRow(
		"SELECT SUM(price) FROM products LIMIT $1 OFFSET $2",
		count, start).Scan(&sum)
